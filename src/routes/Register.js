...
function Register() {
    ...
    const [phoneNumber, updatePhoneNumber] = useState(0)
    ...
    async function handleRegister(e) {
        ...
        if(registerPasswordRef.current.value.length < 5)
        {
            ...
            setregistererror("")
            setLoading(true)
            await signup(registerEmailRef.current.value, registerPasswordRef.current.value, registerNameRef.current.value, phoneNumber).then((data)=>{
                ...
                <Col xs={12} lg={6} className="d-flex justify-content-center flex-column">
                    ...
                    <Form className="login-form" onSubmit={handleRegister}>
                        <Form.Group controlId="name">
                            ...
                            <Form.Control type="text" placeholder="Enter name" ref={registerNameRef} required />
                        </Form.Group>
                        <Form.Group controlId="name">
                            <Form.Label>Phone Number</Form.Label>
                            <Form.Control type="number" placeholder="Enter phone number" onChange={(e)=>updatePhoneNumber(e.target.value)} required isValid={`${phoneNumber}`.length===10 ? true : false}/>
                        </Form.Group>
